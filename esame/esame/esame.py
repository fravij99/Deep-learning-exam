...
def LSTM_model2():
    regressor=tf.keras.models.Sequential()
    regressor.add(LSTM(units = 50, return_sequences = True, input_shape = (3, 1)))
    regressor.add(Dropout(0.2))
...
def finding_best(search_space, trial):    
    return fmin(hyper_func, search_space, algo=tpe.suggest, max_evals=5, trials=trial)


test_setup={'layer_size':2, 'learning_rate':1.0}

model=train(xtrain, ytrain, test_setup)
print('Accuracy model on test set is: ', test(model, xtest, ytest))
